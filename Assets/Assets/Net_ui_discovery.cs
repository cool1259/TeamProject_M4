...
namespace UnityEngine.Networking
{
    ...
    [Obsolete("The high level API classes are deprecated and will be removed in the future.")]
    public class Net_ui_discovery : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            int xpos = 860 + m_OffsetX;
            int ypos = 600 + m_OffsetY;
            ...
            if (m_MsgInBuffer == null && !NetworkClient.active)
            {
             
                if (GUI.Button(new Rect(xpos, ypos, 200, 20), "방 설정") )
                {
                    Initialize();
                }
                return;
            ...
            // GUI.Label(new Rect(xpos, ypos-30, 200, 20), "initialized" + suffix);
            //  ypos += spacing;
            ...
            if (m_Running)
            {
                ...
                if (m_BroadcastsReceived != null && !NetworkClient.active )
                {
            ...
            else
            {
                ...
                {
                    if (GUI.Button(new Rect(xpos, ypos, 200, 20), "공개 방 설정"))
                    {
                        StartAsServer();
                    }
                    ypos += spacing;

                    if (GUI.Button(new Rect(xpos, ypos, 200, 20), "방 찾기"))
                    {
                        StartAsClient();
                    }
                    ypos += spacing;
